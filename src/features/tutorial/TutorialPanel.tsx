...
export function TutorialPanel({
  ...
  return (
    <aside
      className={`tutorial${collapsed ? ' is-collapsed' : ''}`}
      ...
      {collapsed ? null : (
        <>
          ...
          <div className="tutorial__foot">
            {/*
              A checked step reports whether the *engine* agrees it is done, so
              the tick cannot say "correct" about something that would not
              actually run. Next waits for that agreement: skipping the practice
              teaches that the practice is optional. Exit stays one click away in
              the header for anyone who genuinely wants out.
            */}
            {checked ? (
