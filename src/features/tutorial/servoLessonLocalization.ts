...
const TRADITIONAL_CHARACTERS: Readonly<Record<string, string>> = {
  ...
  '会': '會', '留': '留', '狭': '狹', '窄': '窄', '顺': '順',
  // The mirror of the gap in cutterGridLessonLocalization: these came through
  // unconverted, so zh-TW readers were shown simplified words inside otherwise
  // traditional copy. 并 is safe as 並 here because no line uses 合并 (合併).
  '认': '認', '经': '經', '于': '於', '并': '並', '体': '體', '点': '點',
  '盖': '蓋', '范': '範', '围': '圍', '终': '終', '连': '連', '际': '際',
  '径': '徑', '积': '積', '决': '決', '凭': '憑', '画': '畫', '计': '計',
  '应': '應', '断': '斷', '选': '選', '负': '負', '调': '調', '须': '須',
  '样': '樣', '属': '屬', '来': '來', '态': '態', '见': '見', '练': '練',
  '独': '獨', '战': '戰', '话': '話', '释': '釋', '况': '況', '锁': '鎖',
  '规': '規', '则': '則', '项': '項',
  '颠': '顛', '观': '觀', '复': '復', '位': '位', '继': '繼',
