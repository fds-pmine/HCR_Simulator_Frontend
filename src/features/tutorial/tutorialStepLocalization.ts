...
const ZH_TW: Readonly<Record<string, Omit<TutorialStep, 'id'>>> = {
  ...
  absolute: { title: '角度是絕對值，不是相對位移', body: '把積木塊設為「X · 底座偏航」和 120°。意思是「將關節移到 120°」，不是「再轉 120°」。這種語言的每條命令都使用絕對目標；每個角度都是伺服角度，與實機收到的數值相同，回零位（Home）為 90°。', hint: '在下拉選單選擇「X · 底座偏航」，再按數字欄並輸入 120。' },
  test: { title: '「測試」會立即執行', body: '按「測試」後，系統會在數毫秒內評估程式，不會播放即時動畫。嘗試想法的速度不受繪圖效能影響；要觀察動作時再使用「執行」。' },
  head: { title: '頭部會令機械臂停止', body: '底座偏航本身永遠不會碰到頭部。請在第一個積木塊下方加入第二個積木塊「Z · 手肘」並設為 60°，然後按「測試」。手肘不會到達該角度，機械臂會停在最後一個安全姿勢，並指出造成問題的積木塊。這是刻意安排的結果；觀察後請刪除該手肘積木塊。', hint: '留意紅色提示，其中會列出關節及停止位置。' },
  'repeat-noop': { title: '「重複」為何看起來沒有作用', body: '從「控制」拖入「重複」，並把底座偏航積木塊放入其中，然後按「測試」。分數不會改變：第一輪已把關節移到 120°，其餘各輪只會重複相同終點。', hint: '把「設定」積木塊放進「重複」的「執行」區域。' },
  'repeat-sweep': { title: '讓「重複」真正形成掃掠', body: '在「重複」內第一個「設定」下方再加入一個「設定」，並使用不同角度，例如 145°。每輪都會令切刀在頭髮之間來回移動，而不是停在原地。按「測試」觀察分數上升。', hint: '「執行」區域放兩個積木塊：X · 底座偏航 120°，再接 X · 底座偏航 145°。' },
