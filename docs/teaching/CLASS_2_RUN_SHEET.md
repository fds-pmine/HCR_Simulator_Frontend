...
- **A Versus round is single-mode, and the host chooses which. Solo is Servo-Angles-only.** ⟨updated after class 2⟩ The host screen now carries a **Programming mode** control — **Servo Angles / Cutter Grid** (`round-mode-servo`, `round-mode-cutter-grid`, `MatchSetup.tsx`) — and the lobby's rules card names the editor the round will use. Cutter Grid is **offline rooms only**: the browser plans and scores the route, and online rounds keep the option disabled until the backend opens V4 planning to submissions (`hcr-backend/docs/08-CUTTER-GRID.md` §0). Both round modes are pinned by `tests/e2e/versusRoundModes.spec.ts`, and a round never offers a switch once it starts. Solo still pins the mode (`src/features/practice/PracticeRun.tsx`). **On today's script every round was Servo**, and the ten Grid lessons the newcomers missed were not on the critical path. Say that to them at 0:05.
...
- **Anyone in the lobby *could* press Start; now only the host is shown the button.** ⟨updated after class 2⟩ `MatchRegistry::start` still takes a room id and no player id and performs no host check (`rounds.rs:294-310`) — the room code remains the whole permission model, so this is a convention the UI keeps, not a rule the server enforces. What changed is that only the machine that opened the room draws Start; every other screen reads **"Waiting for the host to start"** (`waiting-for-host`, `MatchLobby.tsx`). Releasing the code early is now safe from accidental starts, not from a determined one.
- **Starting counts the room in.** ⟨updated after class 2⟩ Press Start and every screen shows **ROUND STARTING** and a three-second count against the server's clock before the editor appears (`ROUND_COUNTDOWN_MS`, `src/features/match/countdown.ts`; held in `VersusRound.tsx`). It exists because clients poll up to 1.2 s apart, so the round used to begin at a different instant on each laptop. Offline the three seconds are **granted** — a 3-minute round is still 3 minutes of editing (`LocalMatchProvider.startMatch`) — and online they are the first three seconds of the round, because only the server may move a deadline.
...
- **⚠ Next round wipes the scoreboard everybody is reading — and it is now drawn only on your screen.** ⟨updated after class 2⟩ In class 2 the button rendered on *every* player's scoreboard; it is now passed only to the client that opened the room (`VersusRound.tsx`, `MatchScoreboard.tsx`). The route still takes no player header, for the stated reason Start does not: the room code is the whole permission model (`binding.rs:342-347`). Reopening clears `room.entries` server-side (`rounds.rs:284`), so `match_results` errors from that instant onward (pinned at `tests/rounds.rs:302`) and **no photograph, no winner interview and no podium argument survives it.** Season points do survive — each client folds a round into its own table the moment the results land (`VersusRound.tsx`). The sentence at 1:15 is now yours alone to act on rather than a warning to twenty people.
- **The sitting has an ending: End the session.** ⟨updated after class 2⟩ Your scoreboard carries **End the session** (`end-session`) once a round has been folded into the table. It crowns the leader of the **points** table — points, rounds played, wins, the crew league — which is not always the winner of the last round, and that is the point of running a season rather than eight separate games. **Next round** from the champion screen resumes the sitting; **Back to menu** ends it. Use it at 2:00 instead of letting the last scoreboard stand as the result.
...
**Why unnecessary.** Today's competition is Servo-only — every round on the script is, and since class 2 the host may also open an offline round in Cutter Grid (`versusRoundModes.spec.ts`) — and everything else the class does runs through Solo Practice and the Tutorials, all of which are ungated: the tutorial picker renders all three tracks with no lock state (`TutorialPicker.tsx:32-63`) and the Solo card on the home screen has no disabled state (`src/app/HomeScreen.tsx:94-103`), with the router reaching that screen unconditionally (`src/app/GameShell.tsx:127-131`).
...
| Cutter Grid in Versus | ⟨updated after class 2⟩ Now available: the host picks the editor when opening the room, **offline rooms only** (`versusRoundModes.spec.ts`; online stays Servo per `08-CUTTER-GRID.md` §0). Solo still pins the mode (`PracticeRun.tsx`) | On class 2's script Cutter Grid stayed in the Lessons and Tutorial tracks and nothing needed it. For a later class it is now a real option — on the offline build, on a challenge with a certified profile |
