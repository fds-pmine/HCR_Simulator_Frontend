...
| ABI | 五自由度、完全本地、离线 state-to-state `q/v/a`；输出每个采样点的 `q/v/a/j` |
...
固定输入为静止起点 `q=[0,0,0,0,0]`，静止目标 `q=[10,-5,3,1,-2]`，每关节上限 `v/a/j=[20,50,200]`。本地 Worker 返回成功状态 `0`、时长约 `1.1787087811s`、目标端点误差不大于浮点舍入，目标 `v/a` 为零。
...
| `hcr_ruckig_local.wasm` | 132,130 bytes |
| 总计 | 138,174 bytes |
| WASM SHA-256 | `15af8bb3ca17485391d950fd5c34600f66211112f39eba6cb8dac9aa91a7c37d` |
