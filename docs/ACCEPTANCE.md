...
- [ ] 冻结服务端计分参数下，默认参考海龟路径的最终分数 ≥ 80。当前参考路径精确达到目标删发集合，但 15 个高层积木和 23 条兼容 IR 使其在旧参数（`referenceProgramCost=6.25`、`referenceTimeMs=5645`）下为 71.94；按既定“不修改后端计分输入”约束，此项需要产品确认新的服务端 Challenge 计分校准后才能关闭。
