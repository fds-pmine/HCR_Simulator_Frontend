...
- 前端纯领域 Ruckig 分段编排已把相邻 TOPP-RA 边界作为完全共享的 state-to-state `q/v/a` 输入，以 `5ms` 或更密采样验证端点与动态限制；失败只增加同一段的最小时长（`1.1x`，累计不超过 `50x`）。调用方必须注入本地空间认证，逐段复验关节限位、头部、固定 Cartesian 管道、`0.5°`/`voxelSize/16` 联合采样和零接触/允许接触集；空间失败不得被时长扩展重试掩盖。它在 Worker 资产加载和完整 Move 接触聚合认证完成前不是运行时轨迹来源。
